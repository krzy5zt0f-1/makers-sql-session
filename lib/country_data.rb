...
class CountryData
  ...
  def self.population_greater_than_100_million
    result = DatabaseConnection.query("SELECT * FROM countries WHERE population>100 ORDER BY population;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.population_less_than_20_million
    result = DatabaseConnection.query("SELECT * FROM countries WHERE population<20 ORDER BY population;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.population_density_between_50_and_150
    result = DatabaseConnection.query("SELECT * FROM countries WHERE density>=50 AND density<=150 ORDER BY density DESC;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
  ...
  def self.third_lowest_gni_per_capita_in_south_america
    result = DatabaseConnection.query("SELECT * FROM countries WHERE gni=(SELECT MIN(gni) FROM countries WHERE gni>(SELECT MIN(gni) FROM countries WHERE gni>(SELECT MIN(gni) FROM countries WHERE continent='South America') AND continent='South America') AND continent='South America');")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
